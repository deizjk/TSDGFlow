...
def _load_valid_neighbors(filename, expected_shape):
    if not os.path.exists(filename):
        return None
    neighbors_index = pickle.load(open(filename, 'rb'))
    if getattr(neighbors_index, 'shape', None) != expected_shape:
        print('ignore invalid near index from', filename, 'shape', getattr(neighbors_index, 'shape', None))
        return None
...
class SWat_dataset(Dataset):
    def __init__(self, df, label, window_size=60, stride_size=10, k=10, alpha=0.5, train_split=0.8,
        ...
        self.seed = seed
        os.mkdir('save_near_index') if not os.path.exists('save_near_index') else None
        expected_shape = (self.data.shape[0], k)
        neighbor_index_mode = os.environ.get("USD_SWAT_NEIGHBOR_INDEX_MODE", "seed").lower()
        if neighbor_index_mode == "shared":
            filename = (
                "save_near_index/SWat_near{}_train{}_windowsize{}_trainsplit{}_rows{}_features{}.pkl"
                .format(
                    k,
                    self.train,
                    self.window_size,
        ...
        elif neighbor_index_mode == "seed":
            filename = (
                "save_near_index/SWat_near{}_train{}_windowsize{}_trainsplit{}_seed{}_rows{}_features{}.pkl"
                .format(
                    k,
                    self.train,
                    self.window_size,
            ...
            raise ValueError(
                "USD_SWAT_NEIGHBOR_INDEX_MODE must be 'seed' or 'shared', "
                f"got {neighbor_index_mode!r}"
            )
        print("SWaT neighbor index mode:", neighbor_index_mode)
        legacy_filename = "save_near_index/SWat_near{}_train{}_windowsize{}_trainsplit{}_seed{}_622.pkl".format(
            k, self.train, self.window_size, self.train_split, self.seed
        )
        neighbors_index = _load_valid_neighbors(filename, expected_shape)
        if neighbors_index is not None:
            print('load near index from', filename)
        else:
            neighbors_index = _load_valid_neighbors(legacy_filename, expected_shape)
            if neighbors_index is not None:
                pickle.dump(neighbors_index, open(filename, 'wb'))
                print('migrate near index from', legacy_filename, 'to', filename)
